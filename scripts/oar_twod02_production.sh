...
# Results -> results/profile_inversion/twod02/ (does not touch twod01/ or any exp_*/).

#OAR --name twod02_production
#OAR --resource /nodes=1/core=8,walltime=20:00:00
#OAR --project iste-equ-cycle
#OAR --stderr oar_twod02_production.err
#OAR --stdout oar_twod02_production.out
...
python3 -u scripts/invert_twod_profiles.py --skip-done \
    ...
    --outtag twod02 \
    > "$LOGDIR/twod02_production.log" 2>&1
echo "[twod02] finished $(date)"
